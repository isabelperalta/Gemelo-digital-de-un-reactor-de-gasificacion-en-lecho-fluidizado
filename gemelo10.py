...
def build_model_index():
    """
    Construye un índice dict: key_normalizada -> { 'rootfile': (path,name),
                                                   'mod': (path, entry_name),
                                                   'mod1': (path, entry_name) }
    """
    index = {}
        ...
        if os.path.isdir(root):
            for entry in os.listdir(root):
                ...
                if os.path.isdir(p) and folder_has_models(p):
                    ...
                    index.setdefault(key, {})[tag] = (p, entry)
...
def get_outputs_from_index(index, biomasa1_flag: bool):
    ...
    Reglas:
      - Si la carpeta asociada es 'modelo_bnn...' se devuelve el nombre completo de la carpeta.
    ...
    for key in sorted(index.keys()):
        ...
        rep = None
        # priorizar BNN folder name si existe (mod o mod1)
        for tag in ('mod', 'mod1'):
            if tag in info:
                path, entry_name = info[tag]
                if entry_name.lower().startswith("modelo_bnn"):
                    rep = entry_name
                    break
        if rep:
            outputs.append(rep)
        ...
        # sino usar mod > mod1 basename
        if 'mod' in info:
            path, entry_name = info['mod']
            outputs.append(entry_name)
            continue
        if 'mod1' in info:
            path, entry_name = info['mod1']
            outputs.append(entry_name)
            continue
...
def find_model_source_for_output(salida: str, biomasa1_flag: bool):
    ...
    if biomasa1_flag and 'mod1' in info:
        path, entry = info['mod1']
        return {"type": "folder", "path": path, "used": "mod1", "entry": entry}
    if 'mod' in info:
        path, entry = info['mod']
        return {"type": "folder", "path": path, "used": "mod", "entry": entry}
    if 'rootfile' in info:
    ...
    if alt_key != key and alt_key in MODEL_INDEX:
        ...
        if biomasa1_flag and 'mod1' in info2:
            path, entry = info2['mod1']
            return {"type": "folder", "path": path, "used": "mod1", "entry": entry}
        if 'mod' in info2:
            path, entry = info2['mod']
            return {"type": "folder", "path": path, "used": "mod", "entry": entry}
...
def main():
    ...
    # obtener salidas basadas en índice (deduplicadas y con preferencia por modelo_bnn como representante)
    ...
    for salida in outputs:
        ...
        if src["type"] == "folder":
            ...
            # Si la carpeta es BNN (nombre completo como 'modelo_bnn_...'), usamos nombre de carpeta
            if output_dir.lower().startswith("modelo_bnn"):
                # representador para el usuario = nombre completo de la carpeta
                rep_name = output_dir
                ...
                if CLAMP_NON_NEGATIVE:
                    val = max(0.0, val)
                # <-- clave en results: nombre completo de la carpeta (pedido explícito)
