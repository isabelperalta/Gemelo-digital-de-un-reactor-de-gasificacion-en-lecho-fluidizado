...
más robustas para interpretar mlp_det.pt)
"""
import os
import re
import warnings
...
def find_model_source_for_output(salida: str, biomasa1_flag: bool):
    ...
    if biomasa1_flag and folder_has_models(folder_alt):
        return {"type": "folder", "path": folder_alt, "used": "mod1"}
    if folder_has_models(folder_def):
    ...
    for salida in outputs:
        ...
        if src["type"] == "folder":
            ...
            if output_dir.startswith("modelo_bnn"):
                ...
                print(f"-> {salida}: {y_real:.6g}  (usado: {used}, bnn)")
